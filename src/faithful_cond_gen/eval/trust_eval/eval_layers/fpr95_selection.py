...
def evaluate_fpr95_selection(
    ...
    1. Compute threshold t = 95th percentile of real scores
    2. Build G_accept = {g : score(g) <= t}
    3. Build R_match with same condition histogram
    4. Compute KID(G_accept, R_match) - raw or z-normalized based on use_kid_z
    ...
    # Compute z-KID for accepted samples
    gen_feats_np = gen_feats.numpy() if isinstance(gen_feats, torch.Tensor) else gen_feats
    gen_accept = gen_feats_np[accept_indices]
    real_match = real_feats_np[match_real_indices]
    ...
    # Random baseline: sample same number of gen samples with matched histogram
    random_indices = []
    ...
    for cond, count in cond_hist.items():
        ...
        if len(gen_cond_indices) >= count:
            sampled = rng.choice(gen_cond_indices, size=count, replace=False)
        else:
            sampled = gen_cond_indices if gen_cond_indices else []
        random_indices.extend(sampled)

    random_indices = np.array(random_indices, dtype=int)
    gen_random = gen_feats_np[random_indices] if len(random_indices) > 0 else np.array([])

    # Compute KID for random baseline
    kid_random = np.nan
    kid_z_random = np.nan
    if len(gen_random) >= 10 and len(real_match) >= 10:
        k_rand = min(len(gen_random), len(real_match), 500)
        kid_random = calculate_kid_same_m(
            gen_random[:k_rand], real_match[:k_rand], use_cosine=use_cosine
        )
        if use_kid_z and kid_null_stats and np.isfinite(kid_random) and agg_sigma > 1e-10:
            kid_z_random = (kid_random - agg_mu) / agg_sigma
    ...
    if use_kid_z:
        ...
        if results["kid_z_random"] is not None:
            print(f"    z-KID (random baseline): {results['kid_z_random']:.4f}")
    else:
        ...
        if results["kid_raw_random"] is not None:
            print(f"    KID (random baseline): {results['kid_raw_random']:.4f}")
