"""
Comprehensive decile binning + FPR95 diagnostic for posthoc alignment.
...
Usage:
    PYTHONPATH=src uv run python /mnt/pvc/posthoc_debug/comprehensive_eval.py
"""
...
N_BINS = 10
KID_K = 500
N_BOOT = 10          # bootstrap repeats per KID estimate
N_RAND = 10          # random-baseline draws for FPR95
SEED = 42
PCTS = [50, 75, 90, 95]
FIG_DIR = "/mnt/pvc/posthoc_debug/figures"
CACHE_DIR = "/mnt/pvc/posthoc_debug/feature_cache"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
COMPONENTS = ["trust", "realism", "faithfulness"]
MODELS = ["celeba_vanilla_marginal_v1", "celeba_repa_siglip_marginal_v1"]
...
def encode_batch(images, encoder_name):
    ...
    out = []
    with torch.no_grad():
        for s in range(0, len(images), 128):
            e = min(s + 128, len(images))
            out.append(enc(images[s:e].to(DEVICE)).mean(dim=1).cpu())
    del enc
    if DEVICE == "cuda":
...
def l2(x):
    """L2-normalize along dim 1 (for 2-D tensors) or last axis (numpy)."""
    if isinstance(x, torch.Tensor):
        return x / (x.norm(dim=1, keepdim=True) + 1e-12)
    return x / (np.linalg.norm(x, axis=1, keepdims=True) + 1e-12)


# ─── Data loading (with feature caching) ─────────────────────────────────────
...
def load_model_data(model_key):
    ...
    if os.path.exists(cache_path):
        ...
        cache_dir = (f"/mnt/pvc/faithful-cond-gen/outputs/posthoc_alignment/"
                     f"diag/{model_key}/gen_cache")
        pts = sorted(glob.glob(f"{cache_dir}/cond_*.pt"))
        # Per-model stored-tuple order (vanilla_marginal is CK; others alphabetical).
        if model_key == "celeba_vanilla_marginal_v1":
            stored_order = list(CK)
        else:
            stored_order = sorted(CK)
    ...
    # Mapper
    mapper_path = (f"/mnt/pvc/faithful-cond-gen/outputs/posthoc_alignment/"
                   f"mappers/{model_key}/best_mapper.pt")
    ...
    real_siglip = torch.load(
        "/mnt/pvc/faithful-cond-gen/outputs/real_celeba_siglip_meanpatch/train_features.pt",
        map_location="cpu", weights_only=False)
    real_dinov3 = torch.load(
        "/mnt/pvc/faithful-cond-gen/outputs/real_celeba_dinov3_meanpatch/train_features.pt",
        map_location="cpu", weights_only=False)
    raw_train = torch.load(
        f"/mnt/pvc/faithful-cond-gen/outputs/posthoc_alignment/"
        f"raw_hidden/{model_key}/t0.01_hidden.pt",
        map_location="cpu", weights_only=False)
    with torch.no_grad():
    ...
    # Score in each space — score ALL real (not just calibration)
    spaces_cfg = {
    ...
    for name, (gf, rf, rm) in spaces_cfg.items():
        gn, rn = l2(gf), l2(rf)
        cf, cm = filter_feats_and_meta_by_seen_combos(rn, rm, CK, MARGINAL_SEEN_COMBOS)
        comp = fit_trust_scoring_components(cf, cm, CK)
        rg, fg, tg = score_trust_from_components(gn, gen_meta, comp)
        rr, fr, tr = score_trust_from_components(rn, rm, comp)   # ALL real
...
def build_condmatched_real(gen_indices, gen_meta, real_by_cond, seed):
    """Build condition-matched real pool for a set of gen indices.
    Returns (real_pool_indices, cond_hist).
    """
    cond_hist = {}
...
def kid_condmatched(gen_indices, kid_gen, kid_real, gen_meta, real_by_cond,
    ...
    """KID with condition-matched real pool + bootstrap."""
...
def decile_binning(score_arr, kid_gen, kid_real, gen_meta, real_by_cond,
    ...
    """Returns list of (mean_kid, std_kid) per bin."""
    valid = np.isfinite(score_arr)
    results = []

    if mode == "global":
        vi = np.where(valid)[0]
        si = vi[np.argsort(score_arr[vi])]          # ascending = best first
        bsz = len(si) // N_BINS
...
def fpr95_global(gen_score, real_score, kid_gen, kid_real,
    ...
    """FPR95 at each percentile.  Returns dict[pct] -> result."""
    ...
    for pct in PCTS:
        ...
        # Condition-matched real pool (built from accepted histogram)
        ...
        # Random baseline: same condition histogram, same real pool, N_RAND draws
        rand_vals = []
...
def fpr95_per_cond(gen_score, real_score, real_meta,
    ...
    """Per-condition FPR@pct: separate threshold per condition.
    Returns dict[cond_str] -> result.
    """
    # Group real scores by condition (in scoring space)
    real_score_by_cond = {}
    ...
    for cond in sorted(gen_by_cond.keys()):
        ...
        real_kid_idx_c = real_by_cond.get(cond, [])       # DINO features

        if len(real_score_idx_c) < 20 or len(gen_idx_c) < 20 or len(real_kid_idx_c) < 20:
            continue

        # Threshold from real scores of this condition
        ...
        # Accept gen of this condition below threshold
        ...
        # KID: accepted vs real (same condition, no cross-condition matching needed)
        acc_vals = []
        for b in range(N_BOOT):
        ...
        # Random baseline: same count from all gen of this condition
        rand_vals = []
        for rep in range(N_RAND):
...
def plot_decile(decile_by_comp, model_key, space, mode, path):
    ...
    short = model_key.replace("celeba_", "").replace("_marginal_v1", "")
    ax.set_title(f"Decile Binning: KID vs Ranking Mode — {short}/{space} [{mode}]")
    ax.legend()
        ...
        for comp in COMPONENTS:
            ...
            for cond_str in sorted(pc[comp].keys()):
                ...
                print(f"      {cond_str:>20s}: n={r['n_accepted']:4d}/{r['n_total']:4d}  "
                      ...
                      f"Δ={r['delta']:+.5f}  {'✓' if r['delta'] > 0 else '✗'}")
...
if __name__ == "__main__":
    ...
    # Save JSON (convert tuples for serialization)
    def prep(obj):
