"""
Compositional Generalization Analysis for CelebA Faithfulness Scoring.
...
Usage:
    uv run python scripts/run_compositional_analysis_celeba.py --dataset celeba
"""
...
def analyze_single_config(
    ...
    try:
        ...
        result["mean_by_n_attrs"] = {
            n: float(np.mean(v)) for n, v in scores_by_n_attrs.items()
        }
...
def create_summary_report(df: pd.DataFrame, output_dir: Path):
    ...
    report.append(
        "- **fullmodel**: Trained on ALL attribute combinations (joint conditioning)"
    )
    report.append(
        "- **marginalmodel**: Trained only on single-attribute combos (0000, 1000, 0100, 0010, 0001)"
    )
    report.append(
        "- For marginalmodel, any multi-attribute combo is **compositionally OOD**\n"
    )
    ...
    report.append(
        "- **Spearman ρ (vs ΔKID)**: Rank correlation with per-condition delta KID"
    )
    report.append(
        "  - Higher ρ = scorer rankings align with true quality (delta KID)\n"
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="outputs/compositional_analysis"
    )
