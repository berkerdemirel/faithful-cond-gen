"""
Unsupervised GMM: cluster in PCA space, then fit mixture stats in original space.

Approach:
1. Fit GMM in PCA space → get hard cluster assignments
2. Use assignments to compute per-cluster means + shared covariance in ORIGINAL space
3. Score using original-space Mahalanobis

This is apples-to-apples with supervised mixture (same scoring, different grouping).

Usage:
    PYTHONPATH=src uv run python scripts/analyze_gmm_cluster_then_original_space.py
"""
...
def compute_realism_supervised_mixture(gen_feats, gen_meta, real_feats, real_meta, condition_keys, dataset, model):
    """Supervised mixture: components = condition labels."""
...
def fit_gmm_get_assignments(real_feats, n_components, pca_dim=64, random_state=42):
    """
    Fit GMM in PCA space and return hard cluster assignments.
    """
    ...
    gmm = GaussianMixture(
        ...
        covariance_type="full",  # Use full covariance for better clustering
    ...
    # Hard assignments
    assignments = gmm.predict(real_pca)

    return pca, gmm, assignments


def fit_mixture_from_assignments(real_feats, assignments, regularization=1e-5):
    """
    Given cluster assignments, compute per-cluster means and shared covariance
    in ORIGINAL feature space (same as supervised mixture).
    """
    real_normed = normalize_features(real_feats)
    N, D = real_normed.shape
    ...
    for c in unique_clusters:
        mask = assignments == c
        cluster_feats = real_normed[mask]
        mus[c] = cluster_feats.mean(dim=0)
        cluster_sizes[c] = mask.sum()

    print(f"    Cluster sizes: {dict(cluster_sizes)}")

    # Shared (pooled) covariance - LDA style
    # Σ_pooled = Σ_c (n_c - 1) * Σ_c / (N - k)
    S_pooled = torch.zeros(D, D)
    total_df = 0
    ...
    for c in unique_clusters:
        ...
        if n_c > 1:
            centered = cluster_feats - mus[c].unsqueeze(0)
            S_c = (centered.T @ centered) / (n_c - 1)
            S_pooled += (n_c - 1) * S_c
            total_df += n_c - 1

    S_pooled = S_pooled / total_df

    # Add regularization
    S_pooled = S_pooled + regularization * torch.eye(D)

    # Precision
    precision = torch.linalg.inv(S_pooled)

    return {
        "mus": mus,
        "precision": precision,
        "component_keys": list(unique_clusters),
        "n_components": n_components,
        "cluster_sizes": cluster_sizes,
    }


def compute_realism_gmm_original_space(gen_feats, real_feats, mixture_stats):
    """
    Compute realism using GMM-derived mixture stats in original space.
    Same scoring as supervised mixture.
    """
    gen_normed = normalize_features(gen_feats)
    real_normed = normalize_features(real_feats)
    ...
    # Calibrate on real
    energies_real = min_mahal_batch(real_normed)
    real_E_mean = np.mean(energies_real)
    real_E_std = np.std(energies_real) + 1e-8

    # Score gen
    energies_gen = min_mahal_batch(gen_normed)
...
def analyze_model(dataset, model, feature_type="dinov3", pca_dim=64):
    ...
    # 3. Unsupervised GMM: cluster in PCA, fit stats in original space
    n_conditions = len(gen_by_cond)
    print(f"\nFitting unsupervised GMM (k={n_conditions}) in PCA, then stats in original space...")
    pca, gmm, assignments = fit_gmm_get_assignments(real_feats, n_components=n_conditions, pca_dim=pca_dim)
    mixture_stats_gmm = fit_mixture_from_assignments(real_feats, assignments)
    realism_gmm_orig = compute_realism_gmm_original_space(gen_feats, real_feats, mixture_stats_gmm)

    # Also try with k = n_supervised for marginal
    if "marginal" in model and n_supervised != n_conditions:
        print(f"\nFitting unsupervised GMM (k={n_supervised}) in PCA, then stats in original space...")
        _, _, assignments_k5 = fit_gmm_get_assignments(real_feats, n_components=n_supervised, pca_dim=pca_dim)
        mixture_stats_gmm_k5 = fit_mixture_from_assignments(real_feats, assignments_k5)
        realism_gmm_orig_k5 = compute_realism_gmm_original_space(gen_feats, real_feats, mixture_stats_gmm_k5)
    else:
        realism_gmm_orig_k5 = None
    ...
    for cond in sorted(gen_by_cond.keys()):
        ...
        row = {
            ...
            "realism_gmm_orig_k16": float(np.mean(realism_gmm_orig[idx])),
        }
        if realism_gmm_orig_k5 is not None:
            row["realism_gmm_orig_k5"] = float(np.mean(realism_gmm_orig_k5[idx]))
        results.append(row)
    ...
    print("-" * 120)
    cols = ["condition", "is_seen", "delta_kid", "realism_single", "realism_supervised", "realism_gmm_orig_k16"]
    if realism_gmm_orig_k5 is not None:
        cols.append("realism_gmm_orig_k5")
    ...
    print("-" * 70)
    print(f"{'Score':<30} {'Spearman ρ':>12} {'p-value':>10} {'Kendall τ':>12}")
    print("-" * 70)

    scores_to_check = [
        ("realism_single", "Single Gaussian"),
        ("realism_supervised", f"Supervised mixture (k={n_supervised})"),
        ("realism_gmm_orig_k16", f"GMM cluster→orig (k={n_conditions})"),
    ]
    if realism_gmm_orig_k5 is not None:
        scores_to_check.append(("realism_gmm_orig_k5", f"GMM cluster→orig (k={n_supervised})"))
    ...
    for col, label in scores_to_check:
        if col in valid.columns:
            ...
            print(f"{label:<30} {rho:>12.4f} {p:>10.4f} {tau:>12.4f}")
...
def main():
    print("=" * 80)
    print("GMM: CLUSTER IN PCA, FIT STATS IN ORIGINAL SPACE")
    print("=" * 80)
    print("\nApproach:")
    print("  1. Fit GMM in PCA space to get cluster assignments")
    print("  2. Use assignments to compute per-cluster means + shared cov in ORIGINAL space")
    print("  3. Score using original-space Mahalanobis (same as supervised)")
    print()
    ...
    print("=" * 80)
    print(f"\n{'Method':<35} {'Marginal':>12} {'Full':>12}")
    print("-" * 60)
    print(f"{'Single Gaussian':<35} {corr_marginal['realism_single']['rho']:>12.4f} {corr_full['realism_single']['rho']:>12.4f}")
    print(f"{'Supervised mixture':<35} {corr_marginal['realism_supervised']['rho']:>12.4f} {corr_full['realism_supervised']['rho']:>12.4f}")
    print(f"{'GMM cluster→orig (k=16)':<35} {corr_marginal['realism_gmm_orig_k16']['rho']:>12.4f} {corr_full['realism_gmm_orig_k16']['rho']:>12.4f}")
    if "realism_gmm_orig_k5" in corr_marginal:
        print(f"{'GMM cluster→orig (k=5)':<35} {corr_marginal['realism_gmm_orig_k5']['rho']:>12.4f} {'N/A':>12}")

    # Save
    output_dir = Path("outputs/trust_evaluation")
    output_dir.mkdir(parents=True, exist_ok=True)
    df_marginal.to_csv(output_dir / "realism_gmm_orig_space_marginal.csv", index=False)
    df_full.to_csv(output_dir / "realism_gmm_orig_space_full.csv", index=False)
    print(f"\n\nSaved to {output_dir}")
