...
def main(cfg: DictConfig):
    ...
    if generated_path:
        ...
        gen_dataset = GeneratedDataset(
            root_dir=generated_path, dataset_type=dataset_type, transform=enc_transform
        )
        ...
        # Inject Transform
        # Note: We do this after instantiation because DataModules instantiate datasets lazily or in setup()
        # For our lightweight DMs, we assume we can just patch 'transform' on the datasets if they are exposed,
        # or we might need to rely on the DM passing kwargs.
        # The original script strategy was to patch after access.
