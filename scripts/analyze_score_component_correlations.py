...
Breakdown:
- ρ(ΔKID, realism_single)
- ρ(ΔKID, realism_mixture)
- ρ(ΔKID, faithfulness)
- ρ(ΔKID, trust)  [current combined score]

Usage:
    PYTHONPATH=src uv run python scripts/analyze_score_component_correlations.py
"""
...
def compute_faithfulness_per_sample(gen_feats, gen_meta, real_feats, real_meta, condition_keys, dataset, model):
    """
    Compute faithfulness score per sample.

    Faithfulness = Mahalanobis distance to the *correct* condition mean.
    Uses mixture stats where each condition has its own mean.
    """
    mixture_stats = fit_mixture_stats(
        real_feats, real_meta, condition_keys, dataset, model
    )

    if mixture_stats.get("n_components", 0) == 0:
        # Fallback - no mixture, return zeros
        return np.zeros(len(gen_feats))
    ...
    for i in range(N):
        # Get this sample's condition
        cond = tuple(
            int(gen_meta[k][i].item() if isinstance(gen_meta[k][i], torch.Tensor) else gen_meta[k][i])
            for k in condition_keys
        )

        # Find the correct component mean
        if cond in mus:
            mu = mus[cond]
        else:
            # Condition not in mixture - use closest or global mean
            # For marginal model, unseen conditions won't have a mean
            # Use global mean as fallback
            mu = torch.mean(torch.stack(list(mus.values())), dim=0)

        x = gen_normed[i:i+1]
        centered = x - mu.unsqueeze(0)
        energy = torch.sum(centered @ P * centered, dim=1).item()
        energies[i] = energy
...
def compute_realism_mixture_per_sample(gen_feats, gen_meta, real_feats, real_meta, condition_keys, dataset, model):
    """
    Compute mixture Gaussian realism scores.

    Realism = min distance to ANY component mean.
    """
    mixture_stats = fit_mixture_stats(
        real_feats, real_meta, condition_keys, dataset, model
    )
...
def analyze_model(dataset, model, feature_type="dinov3"):
    ...
    realism_mixture, n_components = compute_realism_mixture_per_sample(
    ...
    faithfulness = compute_faithfulness_per_sample(
        gen_feats, gen_meta, real_feats, real_meta, condition_keys, dataset, model
    )

    # Compute trust score (current: equal weight realism + faithfulness)
    # Using single Gaussian realism as the current default
    trust_single = 0.5 * realism_single + 0.5 * faithfulness
    trust_mixture = 0.5 * realism_mixture + 0.5 * faithfulness
    ...
    for cond in sorted(gen_by_cond.keys()):
        ...
        row = {
            ...
            "realism_mixture": float(np.mean(realism_mixture[idx])),
            "faithfulness": float(np.mean(faithfulness[idx])),
            "trust_single": float(np.mean(trust_single[idx])),
            "trust_mixture": float(np.mean(trust_mixture[idx])),
        }
        results.append(row)
    ...
    # Print table
    print(f"\nPer-condition scores (n_components={n_components}):")
    print("-" * 130)
    cols = ["condition", "is_seen", "delta_kid", "realism_single", "realism_mixture", "faithfulness", "trust_single", "trust_mixture"]
    ...
    print("-" * 70)
    print(f"{'Score':<20} {'Spearman ρ':>12} {'p-value':>10} {'Kendall τ':>12}")
    print("-" * 70)

    scores_to_check = [
        ("realism_single", "Realism (single)"),
        ("realism_mixture", "Realism (mixture)"),
        ("faithfulness", "Faithfulness"),
        ("trust_single", "Trust (single)"),
        ("trust_mixture", "Trust (mixture)"),
    ]
    ...
    for col, label in scores_to_check:
        ...
        print(f"{label:<20} {rho:>12.4f} {p:>10.4f} {tau:>12.4f}")
        correlations[col] = {"rho": rho, "p": p, "tau": tau}

    # For marginal, also show seen vs unseen
    if "marginal" in model:
        ...
        for subset_name, subset in [("SEEN", seen), ("UNSEEN", unseen)]:
            if len(subset) >= 3:
                print(f"\n{subset_name} (n={len(subset)}):")
                for col, label in scores_to_check:
                    rho, p = spearmanr(subset[col], subset["delta_kid"])
                    print(f"  {label:<20} ρ = {rho:>7.4f} (p={p:.4f})")
...
def main():
    print("=" * 80)
    print("SCORE COMPONENT CORRELATION ANALYSIS")
    print("=" * 80)
    print("\nQuestion: Which score components correlate with ΔKID?")
    print("If realism correlations are unchanged, mixture won't help.\n")
    ...
    # Summary comparison
    print("\n\n" + "=" * 80)
    print("SUMMARY: Spearman ρ(score, ΔKID)")
    print("=" * 80)
    print(f"\n{'Score':<25} {'Marginal':>12} {'Full':>12}")
    print("-" * 50)
    for col in ["realism_single", "realism_mixture", "faithfulness", "trust_single", "trust_mixture"]:
        rho_m = corr_marginal[col]["rho"]
        rho_f = corr_full[col]["rho"]
        print(f"{col:<25} {rho_m:>12.4f} {rho_f:>12.4f}")

    # Save
    output_dir = Path("outputs/trust_evaluation")
    output_dir.mkdir(parents=True, exist_ok=True)
    df_marginal.to_csv(output_dir / "score_component_correlations_marginal.csv", index=False)
    df_full.to_csv(output_dir / "score_component_correlations_full.csv", index=False)
    print(f"\n\nSaved to {output_dir}")
